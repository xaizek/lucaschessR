...
class TreeMoves(QtWidgets.QTreeWidget):
    ...
    def editaVariantes(self, item, mov):
        ...
        wowner = self.owner
        tablero = wowner.infoMove.tablero
        resp = Variantes.editaVarianteMoves(self.procesador, wowner, tablero.is_white_bottom, fen, lineaPGN)
        if resp:
            una = resp[0]
            if num is None:
...
class WMoves(QtWidgets.QWidget):
    def __init__(self, owner, procesador):
        ...
        self.tb.new(_("Variations"), Iconos.Variantes(), self.variantes)
...
class InfoMove(QtWidgets.QWidget):
    def __init__(self, is_white_bottom):
        ...
        config_board = Code.configuracion.config_board("INFOMOVE", 32)
        self.main_window = self
        self.tablero = Tablero.Tablero(self, config_board)
        self.tablero.crea()
        self.tablero.ponerPiezasAbajo(is_white_bottom)
...
class PantallaArbol(QTVarios.WDialogo):
    def __init__(self, wParent, game, nj, procesador):
        ...
        self.dbCache = UtilSQL.DictSQL(Code.configuracion.ficheroMoves)
        if nj >= 0:
        ...
        self.restore_video(anchoDefecto=869 - 242 + anchoTablero)
        if not dicVideo:
            dicVideo = {
                "TREE_3": 27,
    ...
    def nuevoAnalisis(self, lm):
        ...
        xengine = self.procesador.creaGestorMotor(confMotor, alm.vtime, alm.depth, siMultiPV=True)
