...
class WMap(QTVarios.WDialogo):
    def __init__(self, procesador, mapa):
        ...
        tbWork = QTVarios.LCTB(self, li_acciones, tamIcon=24)
        ...
        tb = QTVarios.LCTB(self, li_acciones, tamIcon=24)
    ...
    def grid_dato(self, grid, fila, oColumna):
        key = oColumna.clave
        return self.workmap.dato(fila, key) if grid.id == "W" else self.workmap.db.dato(fila, key)
...
class WUnSTSMap(QTVarios.WDialogo):
    def __init__(self, owner):
        ...
        # Tablero
        config_board = self.configuracion.config_board("STSMAP", 48)
