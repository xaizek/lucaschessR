...
import Code.Game  # To prevent recursivity in Variations -> import direct
...
class Move:
    ...
    def pgn_translated(self):
        return self.pgnBaseSP()

    def pgnFigurinesSP(self):
        return self.pgnBase

    def pgn_html(self, siFigurines):
    ...
    def analisis2variantes(self, almVariantes, delete_previous):
        ...
        self.variations.analisis2variantes(mrm, almVariantes, delete_previous)
...
def dameJugada(game, position_before, from_sq, to_sq, promotion):
...
class Variations:
    ...
    def add_pgn_variation(self, pgn):
        ...
        ok, p_var = Code.Game.pgn_game(pgn_var)
        if ok:
    ...
    def analisis2variantes(self, mrm, almVariantes, delete_previous):
        if delete_previous:
            self.clear()

        if almVariantes.info_variation:
            name = mrm.name
            if mrm.max_time:
        ...
        tmp_game = Code.Game.Game()
        if almVariantes.best_variation:
            pv_base = self.move_base
            for rm in mrm.li_rm:
                if rm.movimiento() != pv_base:
                    self.analisis2variantesUno(tmp_game, rm, eti_t, almVariantes.one_move_variation, almVariantes.si_pdt)
                    break
        else:
            for rm in mrm.li_rm:
                self.analisis2variantesUno(tmp_game, rm, eti_t, almVariantes.one_move_variation, almVariantes.si_pdt)
