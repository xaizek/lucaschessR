...
class WHistoricoTacticas(QTVarios.WDialogo):
    def __init__(self, main_window, tactica, icono):
        ...
        o_columns.nueva("TIEMPO", "%s - %s:%s" % (_("Days"), _("Hours"), _("Minutes")), 120, centered=True)
    ...
    def grid_doble_click(self, grid, fila, oColumna):
        if fila == 0 and not self.tactica.finished():
            self.entrenar()

    def grid_dato(self, grid, fila, oColumna):
        col = oColumna.clave
        reg = self.li_histo[fila]
        if col == "FINICIAL":
            fecha = reg["FINICIAL"]
    ...
    def entrenar(self):
        if self.tactica.finished():
            ...
            if n >= 0:
                ...
                if "PUZZLES" in reg:
                    menu.separador()
                    menu.opcion("copia%d" % n, _("Copy configuracion from current register"), Iconos.PuntoVerde())
...
class WConfTactics(QtWidgets.QWidget):
    def __init__(self, owner, tactica, ncopia=None):
        ...
        def tb_gen(prev):
            ...
            tb = Controles.TB(self, li_acciones, tamIcon=16, siTexto=False)
            return tb
        ...
        o_col.nueva("NUMERO", _("Repetition"), 80, centered=True)
        ...
        tb = tb_gen("repeat")
        o_col = Columnas.ListaColumnas()
        o_col.nueva("NUMERO", _("Block"), 40, centered=True)
        ...
        tb = tb_gen("penal")
        o_col = Columnas.ListaColumnas()
        o_col.nueva("NUMERO", _("N."), 20, centered=True)
...
class WEditaTactica(QTVarios.WDialogo):
    def __init__(self, owner, tactica, ncopia):
        ...
        QTVarios.WDialogo.__init__(
            self, owner, _X(_("Configuration of %1"), tactica.title), Iconos.Tacticas(), "editaTactica"
        )
